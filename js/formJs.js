...
//Funcao de abrir formulario
	function abrirJanela(){
		$('.btn').click(function(e){
			e.stopPropagation(); // para o elemento ficar permanemte
			$('.bg').fadeIn();
		});
		$('.mobile-menu #contato').click(function(e){// abrindo pelo responsivo
			e.stopPropagation();
			$('.bg').fadeIn();
		})
	}
//Funcao de fechar formulario
	function verificarCliqueFechar(){
		// var el = $('body,.closeBtn'); para clicar em dois lugares para fechar
		var el = $('.closeBtn');
		el.click(function(){
			$('.bg').fadeOut();
		});
...
// Funcao resetar campos
	...
	$('form#form1').submit(function(e){
		...
		if(verificarNome(nome) == false){
			...
			alert('Obrigado pelo acesso: ' + nome).css('background-color','red');
		}
